...
namespace UIOperateInterleaverFinalTest
{
    public class InterleaverScanResult
    {
        ...
        /// 读取扫描数据
        /// </summary>
        /// <param name="path">归零数据路径</param>
        /// <param name="spec">归零数据对应Spec</param>
        /// <param name="errMsg">出错信息</param>
        /// <returns>0--成功 1--文件不存在  2--其他错误，具体看错误信息</returns>
        public static int ReadRefSpec(string path, ref string spec, ref string errMsg)
        {
            try
            {
                if (!File.Exists(path))
        ...
        /// <param name="errMsg">出错信息</param>
        /// <returns>0--成功 1--文件不存在  2--其他错误，具体看错误信息</returns>
        public static int ReadRefPortCount(string path, ref int portCount, ref string errMsg)
        {
            try
            {
                if (!File.Exists(path))
                ...
                if (splits.Length >= 5)
                {
                    portCount = Convert.ToInt32(splits[4]);
                }
                else
                {
                    return 1;
        ...
        /// <param name="errMsg">出错信息</param>
        public static void WritePDLRefData(string path, double[][] resultData,string spec,int portCount, ref string errMsg)
        {
            try
            {
                ...
                string title = string.Format("WL,Power,{0},{1},{2}", DateTime.Now.ToString(),spec,portCount);
                writer.WriteLine(title);
                for (int i = res[0].Length-1; i >=0; i--)
                {                  
                    string line = string.Format("{0},{1},{2}", res[0][i], res[1][i], res[2][i]);
        ...
        public static void CalRawdataByAve(List<double[][]> pdlRawData, double[][] pdlRef, double[][] resultData, ref string errMsg)
        {
            try
            {
                ...
                //四个偏振态下归零数据
                double[] refData1 = pdlRef[1];
                double[] refData2 = pdlRef[1];
                double[] refData3 = pdlRef[1];
                double[] refData4 = pdlRef[1];
                double[] refFre = pdlRef[2];
            ...
            try
            {
                ...
                for (int i = 0; i < Count; i++)
                {
                    ...
                    if (testWL[i] == refWL[i])
                    {
                        ...
                        double pa = Math.Pow(10, (refData1[i]));
                        double pb = Math.Pow(10, (refData2[i]));
                        double pc = Math.Pow(10, (refData3[i]));
                        double pd = Math.Pow(10, (refData4[i]));

                        double p1 = Math.Pow(10, (pdlRawdata1[i]));
                        double p2 = Math.Pow(10, (pdlRawdata2[i]));
                        double p3 = Math.Pow(10, (pdlRawdata3[i]));
                        double p4 = Math.Pow(10, (pdlRawdata4[i]));
                        ...
                        //计算ave IL
                        res[1][i] = sum / 4;
                        res[2][i] = Math.Log10(tMax / tMin);
                        res[3][i] = Math.Log10(tMax);
                        res[4][i] = Math.Log10(tMin);
                        //频率
                        res[5][i] = refFre[i];
                    }
                    else
