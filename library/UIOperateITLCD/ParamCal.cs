...
namespace UIOperateITLCD
{
    public class ParamCal
    {
        ...
        public double CalChannelTestParam(string param, double[][] fre1Data, double[][] fre2Data, double ituFre, double productFre,ref string errMsg)
        {
            try
            {
                for (int i = 0; i < fre1Data.Length; i++)
                {
                    if (fre1Data[i] == null)
                    {
                        errMsg = "无测试数据！";
                ...
                for (int i = 0; i < fre2Data.Length; i++)
                {
                    if (fre2Data[i] == null)
                    {
                        errMsg = "无测试数据！";
                ...
                if (paramSplits[0].ToUpper() == "MAXIL")
                {
                    if (isITU)
                    {
                        return -algorithm.MaxILITU(fre1Data[0].ToList(), fre1Data[2].ToList(), ituFre, ref errMsg);
                    }
                    else if (testPassband.CompareTo(CommonFunction.GetDefaultValue()) != 0)
                    {
                        return -algorithm.MaxIL(fre1Data[0].ToList(), fre1Data[2].ToList(), ituFre, testPassband, ref errMsg);
                    }
                ...
                if (paramSplits[0].ToUpper() == "MINIL")
                {
                    if (isITU)
                    {
                        return -algorithm.MinILITU(fre1Data[0].ToList(), fre1Data[2].ToList(), ituFre, ref errMsg);
                    }
                    else if (testPassband.CompareTo(CommonFunction.GetDefaultValue()) != 0)
                    {
                        return -algorithm.MinIL(fre1Data[0].ToList(), fre1Data[2].ToList(), ituFre, testPassband, ref errMsg);
                    }
                ...
                else if (paramSplits[0].ToUpper() == "PDL")
                {
                    if (isITU)
                    {
                        return algorithm.PDLItu(fre1Data[0].ToList(), fre1Data[4].ToList(), ituFre, ref errMsg);
                    }
                    else if (testPassband.CompareTo(CommonFunction.GetDefaultValue()) != 0)
                    {
                        return algorithm.PDL(fre1Data[0].ToList(), fre1Data[4].ToList(), ituFre, testPassband, ref errMsg);
                    }

                }
                else if (paramSplits[0].ToUpper() == "CD")
                {
                    if (testPassband.CompareTo(CommonFunction.GetDefaultValue()) != 0)
                    {
                        return algorithm.CD(fre2Data[0].ToList(), fre2Data[1].ToList(), ituFre, testPassband, ref errMsg);
                    }
                }
                else if (paramSplits[0].ToUpper() == "PMD")
                {
                    if (testPassband.CompareTo(CommonFunction.GetDefaultValue()) != 0)
                    {
                        return algorithm.PMD(fre1Data[0].ToList(), fre1Data[5].ToList(), ituFre, testPassband, ref errMsg);
                    }

                }
                else if (paramSplits[0].ToUpper() == "GDR")
                {
                    if (testPassband.CompareTo(CommonFunction.GetDefaultValue()) != 0)
                    {
                        return algorithm.GDResult(fre1Data[0].ToList(), fre1Data[1].ToList(), ituFre, testPassband, ref errMsg);
                    }
                }
                else if (paramSplits[0].ToUpper() == "RIPPLE")
                {
                    if (testPassband.CompareTo(CommonFunction.GetDefaultValue()) != 0)
                    {
                        return algorithm.Ripple(fre1Data[0].ToList(), fre1Data[2].ToList(), fre1Data[2].ToList(), ituFre, testPassband, ref errMsg);
                    }
                }
                else if (paramSplits[0].ToUpper() == "SHIFT")
                {
                    if (isITU)
                    {
                        return -algorithm.ShiftITU(fre1Data[0].ToList(), fre1Data[2].ToList(), ituFre, deepth, ref errMsg);
                    }
                    else if (testPassband.CompareTo(CommonFunction.GetDefaultValue()) != 0)
                    {
                        return -algorithm.Shift(fre1Data[0].ToList(), fre1Data[2].ToList(), ituFre, testPassband, deepth, ref errMsg);
                    }
                }
                else if (paramSplits[0].ToUpper() == "ADJ")
                {
                    if (testPassband.CompareTo(CommonFunction.GetDefaultValue()) != 0)
                    {
                        return algorithm.Adj(fre1Data[0].ToList(), fre1Data[2].ToList(), fre1Data[2].ToList(), ituFre, testPassband, productFre, ref errMsg);
                    }
                }
