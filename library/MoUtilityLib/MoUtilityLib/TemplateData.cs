...
namespace MoUtilityLib
{
    ...
    {
        [StrTestTemplate("Atd_GenerateTempletIni.aspx?serialNo=")]
        [StrSaveName("GFQC")]
        Template_GFQC=1,
        [StrTestTemplate("Atd_GenerateTempletIniLaser.aspx?serialNo=")]
        [StrSaveName("OSA")]
        Template_OSA,
        [StrTestTemplate("Atd_GenerateTempletIniManual.aspx?serialNo=")]
        [StrSaveName("1830")]
        Template_1830,
        [StrTestTemplate("Atd_GenerateTempletIniEVOA.aspx?serialNo=")]
        [StrSaveName("EVOA")]
        Template_EVOA,
        [StrTestTemplate("Atd_GenerateTempletIniDC.aspx?serialNo=")]
        [StrSaveName("DC")]
        Template_DC,
        [StrTestTemplate("Atd_GenerateTempletIntEXFO.aspx?serialNo=")]
        [StrSaveName("EXFO")]
        Template_EXFO,
        [StrTestTemplate("Atd_GenerateTempletIntDevice.aspx?serialNo=")]
        [StrSaveName("DEVICE")]
        Template_DEVICE
    }
    ...
    public enum ProcessEnum
    {
        ...
        [StrTestTemplate("&preadjust=1")]
        [StrSaveName("-F")]
        Process_PreAdjust=1,
        [Additional("adjust")]
        [StrTestTemplate("&adjust=1")]
        [StrSaveName("-A")]
        Process_Adjust,
        [Additional("pretest")]
        [StrTestTemplate("&pretest=1")]
        [StrSaveName("-P")]
        Process_Pretest,
        [Additional("test")]
        [StrTestTemplate("")]
        ...
        Process_Test,
        [Additional("test5")]
        [StrTestTemplate("&test5=1")]
        [StrSaveName("-5")]
        Process_Test5,
        [Additional("test6")]
        [StrTestTemplate("&test6=1")]
        [StrSaveName("-6")]
        Process_Test6,
        [Additional("test7")]
        [StrTestTemplate("&test7=1")]
        [StrSaveName("-7")]
        Process_Test7,
        [Additional("test8")]
        [StrTestTemplate("&test8=1")]
        [StrSaveName("-8")]
        Process_Test8,
        [Additional("test9")]
        [StrTestTemplate("&test9=1")]
        [StrSaveName("-9")]
        Process_Test9,
    }
    ...
    public enum TestType
    {
        ...
        [StrSaveName("1")]
        Test_Normal=1,
        [StrTestTemplate("&type=2")]
        [StrSaveName("2")]
        Test_Retest,
        [StrTestTemplate("&type=3")]
        [StrSaveName("3")]
        Test_FinalTest,
    }
    ...
    public class OPTestInfo
    {
        ...
        public void Clear()
        {
            ...
            m_dTestedValue = 0.0;
            m_dCurValue = CommonFunction.GetDefaultValue();
            m_bPass = false;
            m_bTested = false;
            m_lRowIndex = -1;
            m_ParamColumnName = "";
            m_dILRef = CommonFunction.GetDefaultValue();
            m_dRLRef = CommonFunction.GetDefaultValue();
        }

        public OPTestInfo Clone()
        {
            return this.MemberwiseClone() as OPTestInfo;
        }
    }
    ...
    public class GlobalSetting
    {
        ...
        {
            return this.MemberwiseClone() as GlobalSetting;
        }
    ...
    //有扫描归零数据和rawdata的时候数据如何保存？GFF EF曲线等保存数据差异？
    public class TemplateData
    {
        private object m_LockObj = new object();
        private GlobalSetting m_GlobalSetting=new GlobalSetting();
        private List<OPTestInfo> m_AllTestInfo = new List<OPTestInfo>();
        private ProductInfo m_ProductInfo = new ProductInfo();

        private string m_WorkStationID = "";
        private string m_strUserID = "";
        private TemplateTypeEnum m_TemplateType;
        private ProcessEnum m_TestProcess;
        ...
        public bool OpenTemplate(
            ...
            bool bShowData, //是否加载测试过数据
            out string errMsg
            )
        {
            string strAdrress = strSvr + tmplatType.GetStrTestTemplate() + strSN + testProcess.GetStrTestTemplate() + testType.GetStrTestTemplate();
            strAdrress += "&user=" + strUserID;
            strAdrress += "&workstation=" + strGoldSample;
            if (bShowData)
                strAdrress += "&showdata=1";
            ...
            m_WorkStationID = strGoldSample;
            m_strUserID = strUserID;
            m_TemplateType = tmplatType;
            ...
            if (bLoad)
            {
                ...
                m_OpenTime = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
            }
            return true;
        }
        ...
        public OPTestInfo GetTestInfoByIndex(int nIndex)
        {
            lock(m_LockObj)
            {
                if (nIndex < 0||m_AllTestInfo.Count==0)
                    return null;
                return m_AllTestInfo[nIndex].Clone();
            }
        }

        public OPTestInfo[] GetAllTestInfo()
        {
            lock (m_LockObj)
            {
                OPTestInfo[] TestInfoArray = new OPTestInfo[m_AllTestInfo.Count];
                int i = 0;
                ...
                //m_AllTestInfo.CopyTo(TestInfoArray);
        ...
        public void UpdateILRefData(int nIndex, double dILRef)
        {
            lock (m_LockObj)
            {
                if (m_AllTestInfo.Count > nIndex)
                {
                    m_AllTestInfo[nIndex].m_dILRef = dILRef;
                }
            }
        }

        public void UpdateRLRefData(int nIndex, double dRLRef)
        {
            lock (m_LockObj)
            {
                if (m_AllTestInfo.Count > nIndex)
                {
                    m_AllTestInfo[nIndex].m_dRLRef = dRLRef;
                }
            }
        }

        public bool GetAllRef()
        {
            lock (m_LockObj)
            {
                for (int i = 0; i < m_AllTestInfo.Count; i++)
                {
                    if (m_AllTestInfo[i].m_dILRef == CommonFunction.GetDefaultValue() 
                        || m_AllTestInfo[i].m_dILRef == CommonFunction.GetFormatDefaultValue())
                        return false;
                    if (m_AllTestInfo[i].m_TestParam==ParamInfoEnum.OP_ReturnLoss)
                    {
                        if (m_AllTestInfo[i].m_dRLRef == CommonFunction.GetDefaultValue() 
                            || m_AllTestInfo[i].m_dRLRef == CommonFunction.GetFormatDefaultValue())
                            return false;
                    }
                }
                return true;
        ...
        public bool RecordRefData(string strFilePath)
        {
            OPTestInfo[] infoArr;
            lock (m_LockObj)
            {
            ...
            string strWrite = "";
            strWrite += m_ProductInfo.m_TemplateID + "\n";
            foreach (OPTestInfo info in infoArr)
            {
                strWrite += string.Format("{0:0.000},{1:0.000},{2},{3:0.000},{4:0.000}\n", info.m_dWLLeft, info.m_dWLRight,info.m_PortNameForUser, info.m_dILRef, info.m_dRLRef);
            }
            CommonFunction.WriteFile(strFilePath, strWrite);
            return true;
        }

        public bool ReadRefData(string strFilePath)
        {
            ...
            if(refList.Count==0)
                return false;
            //模板不对应
            if (m_ProductInfo.m_TemplateID != refList[0])
            {
                return false;
            }
            //归零数据不完整
            if (m_AllTestInfo.Count != (refList.Count - 1))
                return false;
            for (int i = 0; i < refList.Count - 1; i++)
            {
                ...
                if (strRef.Length < 5)
                    return false;
                if(m_AllTestInfo[i].m_dWLLeft.CompareTo(Convert.ToDouble(strRef[0]))!=0)
                    return false;
                if (m_AllTestInfo[i].m_dWLRight.CompareTo(Convert.ToDouble(strRef[1])) != 0)
                    return false;
                if (m_AllTestInfo[i].m_PortNameForUser != strRef[2])
                    return false;
                m_AllTestInfo[i].m_dILRef = Convert.ToDouble(strRef[3]);
                m_AllTestInfo[i].m_dRLRef = Convert.ToDouble(strRef[4]);
            }
            return true;
        }

        //1--SN 号有误
        public int SaveDataToAMTS(string strSN,string strUrl,out string errMsg,bool bLighted=false)
        {
            string strErr = "";
            errMsg = strErr;
            if (m_ProductInfo.m_SN != strSN)
            {
                errMsg = "SN号有误，请检查！";
                return 1;
            }
            ...
            string strPrePort = "";
            string strSave = "";
            ParamRuleEnum preRule = ParamRuleEnum.PARAM_DEFAULT;
            try
            {
                for(int i=0;i<m_AllTestInfo.Count;i++)
                {
                    OPTestInfo info = m_AllTestInfo[i];
                    //照光后--低温  照光前--常温
                    if ((m_TestProcess == ProcessEnum.Process_Adjust || m_TestProcess == ProcessEnum.Process_PreAdjust) && bLighted)
                    {
                        strTmpt = "1";
                    }
                    else
                    {
                        //0--常温  1--低温  2--高温
                        double dTmpt = m_AllTestInfo[0].m_dTemperature;
                        if (dTmpt >= 20 && dTmpt <= 30)
                            strTmpt = "0";
                        else if (dTmpt < 20)
                            strTmpt = "1";
                        else if (dTmpt > 30)
                    ...
                    {
                        strSave = "<AMTS>";
                        strSave += "<SN>" + m_ProductInfo.m_SN + "</SN>";
                        strSave += "<TT>" + m_TestType.GetStrSaveName() + "</TT>";
                        strSave += "<TEMPLET>" + m_ProductInfo.m_TemplateID + "</TEMPLET>";
                        strSave += "<VER>" + m_ProductInfo.m_Version + "</VER>";
                        strSave += "<PN>" + m_ProductInfo.m_ProductPN + "</PN>";
                        strSave += "<SPEC>" + m_ProductInfo.m_SpecNO + "</SPEC>";
                        strSave += "<USER>" + m_strUserID + "</USER>";
                        strSave += "<COMPUTER>" + m_WorkStationID + "</COMPUTER>";
                        strSave += "<DN>" + m_ProductInfo.m_DeviceNO + "</DN>";
                        if (i == 0)
                        {
                            strSave += "<START>" + m_OpenTime + "</START>";
                        }
                        strSave += "<DATE>" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "</DATE>";
                        strSave += "<SOFTWARE>" + m_TemplateType.GetStrSaveName() + m_TestProcess.GetStrSaveName() + "</SOFTWARE>";
                        strSave += "<TEMP VALUE=\"" + strTmpt + "\">";
                    }
                    if ((strPrePort != info.m_PortNameForAMTS) || (strPreTmpt != strTmpt))
                    {
                        strSave += "<PORT VALUE=\"" + info.m_PortNameForAMTS + "\">";
                    }
                    strPrePort = info.m_PortNameForAMTS;
                    strPreTmpt = strTmpt;
                    //WL DB 有何差别
                    if (preRule != info.m_ulParamType)
                    {
                        if (info.m_ulParamType == ParamRuleEnum.PARAM_DB)
                        {
                            strSave += "<DB VALUE=\"" + info.m_dSettingValue.ToString() + "\">";
                        }
                        else if (info.m_ulParamType == ParamRuleEnum.PARAM_WL)
                        {
                            strSave += "<WL LEFT=\"" + info.m_dWLLeft + "\" RIGHT=\"" + info.m_dWLRight + "\">";
                        }
                    }
                    preRule = info.m_ulParamType;
                    if (info.m_dCurValue.CompareTo(CommonFunction.GetDefaultValue()) != 0 &&info.m_dCurValue.CompareTo(CommonFunction.GetFormatDefaultValue()) != 0)
                        strSave += "<" + info.m_TestParam.GetStrSaveName() + ">" + info.m_dCurValue.ToString("#0.000") + "</" + info.m_TestParam.GetStrSaveName() + ">";
                    //下一个测试项是否是相同温度、相同端口、相同类型参数
                    if (i < m_AllTestInfo.Count - 1)
                    {
                        OPTestInfo nextInfo = m_AllTestInfo[i + 1];
                        if ((info.m_ulParamType != nextInfo.m_ulParamType)
                            || (info.m_PortNameForAMTS != nextInfo.m_PortNameForAMTS)
                            ||(info.m_dTemperature!=nextInfo.m_dTemperature))
                        {
                            if (info.m_ulParamType == ParamRuleEnum.PARAM_DB)
                            {
                                strSave += "</DB>";
                            ...
                            preRule = ParamRuleEnum.PARAM_DEFAULT;
                        }
                        if ((info.m_PortNameForAMTS != nextInfo.m_PortNameForAMTS) || (info.m_dTemperature != nextInfo.m_dTemperature))
                        {
                            strSave += "</PORT>";
                        }
                        if (info.m_dTemperature != nextInfo.m_dTemperature)
                        {
                            strSave += "</TEMP>";
                            strSave += "</AMTS>";
                            string[] args = new string[1];
                            args[0] = strSave;
                            string dataFileName = m_ProductInfo.m_SN + "_" + strTmpt + ".ini";
        ...
        private bool CheckPassOrFail(int nIndex)
        {
            lock (m_LockObj)
            {
                if (m_AllTestInfo.Count > nIndex)
                {
                    return CheckPassOrFail(m_AllTestInfo[nIndex]);
                }
            }
            return true;
        }

        private bool CheckPassOrFail(OPTestInfo info)
        {
            double dTestValue = info.m_dCurValue;
            bool bPass = MeetCritereon(dTestValue, info.m_dLowestCriterion)
                & MeetCritereon(dTestValue, info.m_dHighestCriterion);
            info.m_bPass = bPass;
            return bPass;
        }
        ...
        private bool ParserParam(ref IniParser tpParser, string strTmpt, string strPort, string strType, int nIndex, out List<OPTestInfo> ParamInfo)
        {
            ...
            foreach (char ch in strCode)
            {
                ...
                //读出测试过数据，并显示状态
                if (getParamInfo.m_dTestedValue.CompareTo(CommonFunction.GetDefaultValue()) != 0
                    && getParamInfo.m_dTestedValue.CompareTo(CommonFunction.GetFormatDefaultValue()) != 0)
                {
                    getParamInfo.m_dCurValue = getParamInfo.m_dTestedValue;
                    getParamInfo.m_bTested = true;
