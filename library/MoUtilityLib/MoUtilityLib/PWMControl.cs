...
namespace MoUtilityLib
{
    ...
    public class PWMControl
    {
       ...
       // private  MessageBasedSession m_BaseSession;
        ...
        public bool ReadComBytes(long nReadCount,out byte[] byteres, out string errMsg,long nAlreadyRead=0)
        {
            //byte[] res = new byte[nReadCount];
            byteres = new byte[nReadCount];
            errMsg = "";
            try
            {          
                long actualCount = 0;
                ...
                {
                    if (actualCount != nReadCount)
                    {
                        Thread.Sleep(20);
                        nReadCount -= (int)actualCount;
                        byte[] res2;
                        if (!ReadComBytes(nReadCount, out res2, out errMsg, actualCount))
                            return false;
                        for (int i = (int)nAlreadyRead; i < res2.Length; i++)
                        {
                            byteres[i] = res2[i];
                        }
                    }
        ...
        public bool ReadComString(int nReadCount, out string strRes, out string errMsg, long nAlreadyRead=0)
        {
            errMsg = "";
            strRes = "";
            try
        ...
        {
            double dPower=CommonFunction.GetDefaultValue();
            if (m_PWMType == PWMTypeEnum.PWM_1830 || m_PWMType == PWMTypeEnum.PWM_OPLK1830)
            {
                try
                {
            ...
            {
                double dCH1Value = CommonFunction.GetDefaultValue();
                double dCH2Value = CommonFunction.GetDefaultValue();
                string errMsg;
        ...
        public bool GetPower_PDL( byte bChannelIndex, int maxSamples, double dblref,out string errMsg,int nSensor=0, int nCH=0)
        {
            ...
            try
            {
                ...
                for (int index = 0; index < maxSamples; index++)
                {
                    ...
                    if (dPowerNew == CommonFunction.GetDefaultValue() || dPowerNew == CommonFunction.GetFormatDefaultValue())
                    {
                        nerrCount++;
                        maxSamples++;
                        if (nerrCount > 10)
        ...
        public bool StartJHReadPower(out string errMsg)
        {
            ...
            m_BaseSession.Flush(IOBuffers.Read, true);
            m_BaseSession.Flush(IOBuffers.Write, true);
            m_BaseSession.RawIO.Write(send_buf, 0, 7);
            Thread.Sleep(20);
            byte[] byteRes;
            bool bSuccess = ReadComBytes(9,out byteRes, out errMsg);
        ...
        public bool ReadJHPowerValue(ref double dblValue1, ref double dblValue2, out string errMsg)
        {
            ...
            if (bSuccess&&bres.Length==9)   //读取成功
            {
                ...
                if (bres[8] == JHCheckXor(bres, 1, 7) && bres[1] == 0x11)
                {
                    ...
                    dblValue1 = CommonFunction.GetDefaultValue();
            }
            else
                return false;
            ...
            if (bSuccess && bres.Length == 9)   //读取成功
            {
                ...
                if (bres[8] == JHCheckXor(bres, 1, 7) && bres[1] == 0x12)
                {
                    ...
                    dblValue2 = CommonFunction.GetDefaultValue();
            }
            else
                return false;
