        ...
        public double CalChannelTestParam(string param, double[][] resData, double[][] borderData, double ituFre, double productFre,ref string errMsg)
        {
            try
            {
                ...
                else if (paramSplits[0].ToUpper() == "ADJ")
                {
                    if (testPassband.CompareTo(CommonFunction.GetDefaultValue()) != 0)
                    {
                        return algorithm.Adj(resData[5].ToList(), resData[4].ToList(), resData[3].ToList(), ituFre, testPassband, productFre, ref errMsg);
                    }
                }
                ...
                else if (paramSplits[0].ToUpper() == "STOPBAND")
                {
                    if (testPassband.CompareTo(CommonFunction.GetDefaultValue()) != 0 && deepth.CompareTo(CommonFunction.GetDefaultValue()) != 0)
                    {
                        return algorithm.StopBand(resData[5].ToList(), resData[3].ToList(), resData[1].ToList(), ituFre, testPassband, deepth, ref errMsg);
                    }
                }
        ...
        public double CalPortParam(string param,string temperature,string port, List<SamePortParamData> records, ref string errMsg)
        {
            try
            {
                ...
                else if (paramSplits[0].ToUpper() == "UNIPDL")
                {
                    ...
                    if (maxRawdatas != null && minRawdatas != null)
                    {
                        //return algorithm.UniformityPDL(maxRawdatas, minRawdatas, ref errMsg);
                        return CommonFunction.GetDefaultValue();
                    }
                }
                else if (paramSplits[0].ToUpper() == "WDL")
                ...
                else
                {
                    ...
                    if (rawdatas != null)
                    {
                        if (paramSplits[0].ToUpper() == "STOPBAND" || paramSplits[0].ToUpper() == "MAXIL")
                        {
                            // 计算最大值
                            double max;
                            double min;
