...
namespace DeviceControl
{
    public class PowermeterJH : MolexUtility.Device.IPowermeter
    {
        ...
        /// 读写指令同步事件
        /// </summary>
        private AutoResetEvent comReceiveEvent;

        /// <summary>
        /// 用于存放读取的指令，两条线程都需要访问，注意线程同步
        /// </summary>
        private List<string> comResponds = new List<string>();
        ...
        {
            if (comResponds.Count > 0)
            {
                string respond = comResponds[0];
                comResponds.RemoveAt(0);
                return respond;
            }
            return "";
        }
        ...
        private int Open(ref string errMsg, string com, string baudrate)
        {
            try
            {
                comReceiveEvent = new AutoResetEvent(false);
                int baudrateInt = 0;
        ...
        /*public int ReadPowerAvg(ref string errMsg, out List<double> powerArray, int avgSample = 1, bool isGetAllChannel = false, string specialChannel = "0")
        {
            ...
            dPower.Clear();
            GetPMPower(ref dPower, ref errMsg);
            powerArray = dPower;
            return 0;
            ...
            lock (lockObj)
            {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        for (int index = 0; index < avgSample; index++)
                        {
                            dPower.Clear();
                            if (GetPMPower(ref dPower, ref errMsg) == 0)
                            {
        ...
        public virtual void BaseSession_ThreadReadEvent(string readStr, string errMsg)
        {
            reserveRead += readStr;
            bool isNotice = false;
            char begin = Convert.ToChar(0x55);
            int beginIndex = reserveRead.IndexOf(begin);
            while (beginIndex != -1|| reserveRead.Length == 9)
            {
                if (reserveRead.Length >= 9 + beginIndex)
                {
                    string respond = reserveRead.Substring(beginIndex, 9);
                    AddResponds(respond);
                    comReceiveEvent.Set();
                    //isNotice = true;
                    reserveRead = reserveRead.Substring(beginIndex + 9, reserveRead.Length - (beginIndex + 9));
                    beginIndex = reserveRead.IndexOf(begin);
                }
                else if(reserveRead.Length == 9)
                {
                    string respond = reserveRead;
                    AddResponds(respond);
                    comReceiveEvent.Set();
                    //isNotice = true;
                    reserveRead = "";
        ...
        private int SetWavelength(double centerWL,byte channelIndex, ref string errMsg)
        {
            try
            {
                ...
                while (!comReceiveEvent.WaitOne(TimeSpan.FromSeconds(2)))
                {
                    errMsg = "嘉惠功率计 error:接收回复指令超时（2S）！\r";
                    return 1;
                }

                char[] resultBuf = new char[9];
                string ackCmd = GetResponds();
                ClearResponds();
                resultBuf = ackCmd.ToArray();
        ...
        private int SetUnits(ref string errMsg, byte unitsIndex, byte channelIndex)
        {
            try
            {
                ...
                sendBuf[5] = 0x0;
                sendBuf[6] = GetCheckXor(sendBuf, 1, 5);
                //string strCMD = System.Text.Encoding.Default.GetString(sendBuf);
                //baseSession.WriteSerialString(strCMD, ref errMsg);
                baseSession.WriteSerailBytes(sendBuf,ref errMsg);

                while (!comReceiveEvent.WaitOne(TimeSpan.FromSeconds(10)))
                {
                    errMsg = "嘉惠功率计 error:接收回复指令超时（2S）！\r";
                    return 1;
                }
        ...
        private int ReadRespond(ref string errMsg,ref string ack)
        {
            comReceiveEvent.Reset();
            if (!comReceiveEvent.WaitOne(TimeSpan.FromSeconds(2)))
            {
                errMsg = "嘉惠功率计 error:接收回复指令超时（2S）！\r";
                return 1;
            }
        ...
        private int ReadPowerValue(ref List<double> powerArray, ref string errMsg)
        {
            try
            {
                ...
                send_buf[5] = 0x0;
                send_buf[6] = 0x07;
                //string strCMD = System.Text.Encoding.Default.GetString(send_buf);
                //baseSession.WriteSerialString(strCMD, ref errMsg);
                baseSession.WriteSerailBytes(send_buf, ref errMsg);
                //Thread.Sleep(100);
                char[] resultBuf = new char[9];
                ...
                string ackCmd = "";
                int res = ReadRespond(ref errMsg, ref ackCmd);
                if (res != 0)
                    return res;                    
                resultBuf = ackCmd.ToArray();
                ...
                if (resultBuf[8] == GetCheckXor(resultBuf, 1, 7) && resultBuf[1] == 0x11)
                {
                    ...
                    byValue[0] = Convert.ToByte(resultBuf[4]);
                    byValue[1] = Convert.ToByte(resultBuf[5]);
                    byValue[2] = Convert.ToByte(resultBuf[6]);
                    byValue[3] = Convert.ToByte(resultBuf[7]);
                ...
                resultBuf = new char[9];
                ackCmd = GetResponds();
                if(ackCmd=="")
                {
                    while (!comReceiveEvent.WaitOne(TimeSpan.FromSeconds(4)))
                    {
                        errMsg = "嘉惠功率计 error:接收回复指令超时（2S）！\r";
                        return 1;
                    }
        ...
        /// <returns>校验值</returns>
        private byte GetCheckXor(char[] buffer, int startIndex, int stopIndex)
        {
            if (buffer.Length == 0)
                return 0;
            byte byRes = Convert.ToByte(buffer[startIndex]);
