...
public:
  ...
  int TLSScan(BOOL bDoPDL,BOOL bDoRef, double dWLStart, double dWLStop, double dStep,CString strfilefullname);//归零的时候不需要传入文件名，测试的时候传入全路径带名称
  int ConnectServer(stClentTestingConfig  m_testinfo);    //传入连接结构体，结构体参数如下
  void CloseSocket();
  /*
